...
Он заключается в том, что перед тем как делать вычисления, данные приводятся к виду, с которым удобно работать. Например длина строки для 
substr может иметь следующие неудобные значения:
...
const substr = (str, index = 0, length = getLength(str)) => {
  if (length === 0 || index > getLength(str) - 1) {
    return '';
  }
  if (length < 0) {
    return substr(str, index, length = 1);
  }
  if (index < 0) {
    return substr(str, index = 0, length);
  }
  if (length + index > getLength(str)) {
    return substr(str, index, length = getLength(str) - index);
  }
  let result = '';
  if (index === 0) {
    for (let count = index; count < length; count += 1) {
      result += str[count];
    } return result;
  }
  if (index > 0) {
